...
description: 專案庫工具，依據輸入內容生成指定的指南文件
user-invocable: true
...
+ GUIDELINE_FILENAME 為 {{ARGUMENTS}} 中的指南文件檔名。
+ GUIDELINE_REFERENCES 為 {{ARGUMENTS}} 中的參考文件描述。
+ GUIDELINE_DESC 為 {{ARGUMENTS}} 中的詮釋指南描述。

---

### 2. 載入指南

+ GUIDELINE_PATH : `.codekit/memory/guideline/{{CONSTITUTION_FILENAME}}`
+ GUIDELINE_TEMPLATE_PATH : `.claude/templates/guideline-template.md`
...
+ 依據 {{GUIDELINE_REFERENCES}} 載入參考文件

### 5. 詮釋內容

+ 詮釋 {{GUIDELINE_DESC}} 的內容
+ 若描述內容指定使用 {{GUIDELINE_REFERENCES}}，務必使用參考文件，不可自行解釋文字。
